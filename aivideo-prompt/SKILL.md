...
When the user references the `Yunwuxin-666/AIvideo-prompt` style, use the four-section structure in `references/aivideo-output-format.md`.

For commercial films, first build an ad brief before writing shots. Use `references/commercial-film-settings.md`. For Apple-style commercials, also use `references/apple-style-commercials.md`.
...
- **Apple-style short**: only when explicitly requested, use a clear human problem, one elegant tool action, restrained product reveal, dry humor or quiet emotional release. Read `references/apple-style-commercials.md` and `references/apple-storyboard-production-rules.md`; if deeper Apple patterns are needed, also read `../storyboard-video-prompts/references/apple-ad-patterns.md` when available.
...
When the user asks for Seedance 2.5, read `references/seedance-2-5-profile.md`. Apply the Seedance structure above, then adapt to the officially released 2.5 capabilities:
...
For Apple-style storyboard revisions or generated-video review, read `references/apple-storyboard-production-rules.md` and `references/apple-visual-direction.md`. Load them only for an explicit Apple-style route.
