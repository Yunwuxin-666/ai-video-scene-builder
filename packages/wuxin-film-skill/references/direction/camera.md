...
- For shot size, lens, angle, movement, camera settings, and continuity choices, read the same-directory `camera-language.md`.
- For complete storyboard coverage, one-take vs cut decisions, macro-to-detail structure, and creative camera-role selection, read the same-directory `coverage-patterns.md`.
- For scene-specific camera coverage, read the same-directory `camera-scene-rules.md`.
- For copy-ready camera-only prompt formats, read the same-directory `camera-prompt-templates.md`.
